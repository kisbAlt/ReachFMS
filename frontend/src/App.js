...
function App() {
    ...
    const frameNum_ref = useRef();
    frameNum_ref.current = frameNum;

    const [useTiff, setUseTiff] = useState(true);
    const tiff_ref = useRef();
    tiff_ref.current = useTiff;

    const hwnd_ref = useRef()
    hwnd_ref.current = selectedHwnd;
    const refresh_ref = useRef()
    refresh_ref.current = selectedRefreshRateMs;

    const instr_ref = useRef();
    instr_ref.current = selectedInstrument;
            ...
            var status = getStatus().then(async (res) => {
                ...
                for (let i = 0; i < fnd.length; i++) {
                    if (fnd[i].instrument === dispParam) {
                        instrumentMenuSelectHandler(fnd[i])
                    }
                }
            });
        }
        ...
        return (
            ...
            <div style={{
                ...
                {notifMessage !== "" && (<div style={{
                    ...
                    fontWeight: "500",
                    padding: "10px",
                    backgroundColor: notifMessage.includes("error") ? ("rgb(255,0,0,0.8)") : ("rgb(0,128,0,0.8)")
                }}>
                    <p style={{marginTop: "10px", marginBottom: "10px", fontWeight: "bolder"}}>
                        {notifMessage}
                ...
                {!fullScreenMode && (<div style={{
                    ...
                    <div className={"menuwrapper"} style={{
                        ...
                        <div onClick={() => {
                            instrumentMenuSelectHandler({"hwnd": 0, "instrument": "Settings"})
                        }} className={"menudiv"} style={{
                            color: `${selectedInstrument === "Settings" ? ("white") : ("darkgray")}`,
                            borderBottom: `${selectedInstrument === "Settings" ? ("2px solid #1ab164") : ("")}`,
                            paddingRight: "5px",
                        }}>
                            <p>Settings</p>
                        </div>
                        ...
                        {foundInstrumentObjects.map((item, i) => {
                            return item.excluded ? ("") : (
                                <div key={i} onClick={() => {
                                    ...
                                    color: `${selectedInstrumentId === item.hwnd ? ("white") : ("darkgray")}`,
                                    borderBottom: `${selectedInstrumentId === item.hwnd ? ("2px solid #1ab164") : ("")}`,
                                }}>
                                    <p>{item.instrument}</p>
                                </div>
                            )
                ...
                {!fullScreenMode && (<svg width="152.6" onClick={() => {
                    window.open("https://www.buymeacoffee.com/kisb", "_blank");
                }} className="buymeacoffee" height="42.87" viewBox="0 0 545 153" fill="none"
